...
`Actors`, `Workers`, `WorkerPools`, `ActorTemplates`. Внесены по брифингу
`terminology-researcher` от 2026-08-10 (все `confidence: high`), исходная
статья — InfoQ «Pods as Workers, Not Agents».
...
| Worker (Agent Substrate) | Worker | Не переводить. Единица-исполнитель, 1:1 с Pod. **Не путать с Worker node** — это «рабочая нода / worker-нода» (раздел 10). В одном абзаце держать разграничение явно |
...
| NodePool | NodePool | Оригиналом, сноска не нужна: устоявшийся термин облачных провайдеров (GKE, VK Cloud, Yandex Cloud), в русских текстах также «пул нод» |
...
| Внутренний skill kubernetes-vk-cloud-expert | Терминология Managed Kubernetes от VK Cloud (прежнее имя — Cloud Containers) |
