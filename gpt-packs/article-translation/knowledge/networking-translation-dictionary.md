...
description: |
  ...
  с английского на русский. Покрывает TCP/IP, маршрутизацию,
...
1. **Аббревиатуры протоколов и сервисов — оригинал.** IP, TCP, UDP, BGP, OSPF, DNS, DHCP, VXLAN, MPLS, IPSec, TLS, ACL, NAT, CIDR, VPC, WAF, IDS, IPS, DDoS. Расшифровка узкой аббревиатуры при первом упоминании — сноской переводчика («… — Прим. перев.»), не в скобках: `VTEP[^1]` + `[^1]: VXLAN Tunnel Endpoint, конечная точка VXLAN-туннеля. — Прим. перев.`
...
4. **Имена облачных сервисов — оригинал.** `VPC`, `NAT Gateway`, `Internet Gateway`, `Transit Gateway` — пишутся как в документации провайдера. Общие понятия внутри облака переводятся: `subnet` — «подсеть», `route table` — «таблица маршрутизации»; имя объекта в коде и консоли остаётся как есть.
...
| Latency | задержка | «Сетевая задержка». Не «латентность»: калька |
| Jitter | джиттер | — |
...
| Subnet | подсеть | Склоняется: подсети, подсетей. В облаке имя объекта `Subnet` — в коде и консоли оригинал, в нарративе «подсеть» |
...
| Multicast | многоадресная передача / мультикаст / multicast | Также «групповая передача» |
| Broadcast | широковещательная передача / широковещание / broadcast | — |
| Unicast | одноадресная передача / юникаст / unicast | Не «однонаправленная»: unicast — доставка одному адресату, направление тут ни при чём |
...
| TTL (Time To Live) | TTL | Время жизни: DNS-записи, кэша CDN, IP-пакета. Не переводится |
...
| Route | маршрут / route | «Сетевой маршрут», «таблица маршрутизации» |
...
| Convergence (routing) | сходимость | «Сходимость BGP/OSPF», «время сходимости». Не «конвергенция» |
| Network convergence (converged networks) | конвергенция сетей | Объединение голоса, видео и данных в одной сети — единственный случай для «конвергенции» |
...
| Route Table | таблица маршрутизации | Имя объекта в облаке (`Route Table`) — оригинал в коде и консоли |
| Security Group | Security Group / группа безопасности | — |
| Network ACL (NACL) | NACL / сетевой ACL | — |
| Elastic IP (EIP) | Elastic IP / EIP | AWS |
| Floating IP | floating IP / плавающий IP | OpenStack |
...
| Circuit breaking | circuit breaking | Латиницей; при первом упоминании — сноска переводчика «размыкатель цепи». «Прерывание цепи» — НЕ использовать |
| Circuit breaker | circuit breaker | Латиницей; при первом упоминании — сноска переводчика «предохранитель, автоматический выключатель» |
| Rate limiting | ограничение частоты запросов | Глобальное / локальное — global / local rate limiting |
| Bare-metal line rate | линейная скорость на bare-metal | При первом упоминании — сноска переводчика «обработка на полной скорости интерфейса» |
...
| Failover | аварийное переключение / failover | «Аварийное переключение на резерв»; в техтексте допустим оригинал |
...
| Edge node | периферийный узел / edge-узел | В техтексте допустима «edge-нода» |
...
| Streaming | стриминг | Видеостриминг: HLS, DASH |
...
| Honeypot | ханипот | Ловушка-приманка для атакующих; «приманка» — пояснение при первом упоминании, не термин |
...
| English | Русский | Комментарий |
|---------|---------|-------------|
| Zero Trust | Zero Trust / нулевое доверие | При первом упоминании — сноска переводчика |
...
| to encrypt / decrypt | зашифровать / расшифровать | Совершенный вид; несовершенный — «шифровать / расшифровывать». «Дешифровать» — про взлом без ключа, не для decrypt |
...
| to throttle | ограничивать частоту запросов / троттлить | «Дросселировать» — только про полосу канала (traffic shaping) |
| to fail over | выполнить аварийное переключение / переключиться на резерв | — |
...
Перевод одинаковый — «пропускная способность», но в контексте сравнения теории и факта нужно уточнять словами: «теоретическая пропускная способность канала — 1 Гбит/с, фактическая — 800 Мбит/с». Если важно сохранить оригинальные термины — сноска переводчика, не скобки.
...
- [ ] **Bandwidth ≠ Throughput** — разница сохранена, при необходимости — уточнение словами или сноской переводчика
- [ ] **Latency** переведено как «задержка», не «латентность» и не «отставание»
...
3. Проверь русскоязычную документацию облачных провайдеров (Selectel, Yandex Cloud) — особенно для cloud networking.
4. Если нигде нет — оставь **в оригинале** и при первом упоминании дай сноску переводчика: `Term[^1]` … `[^1]: Устоявшегося русского перевода нет, оставлен оригинал. — Прим. перев.` Инлайн-заметки и пометки в скобках не используются — только сноска.
...
При конфликте терминов — приоритет за словарём, более релевантным теме статьи. Полные правила приоритизации — в `translation-standards.md`, секция «Приоритеты при множественной активации».
