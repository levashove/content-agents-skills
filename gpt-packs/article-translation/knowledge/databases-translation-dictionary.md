...
description: |
  ...
  на русский. Покрывает реляционные СУБД (PostgreSQL, MySQL/MariaDB),
...
5. **Команды CLI — оригинал, строчная буква, не склоняются.** `psql`, `mysql`, `clickhouse-client`, `pg_dump`, `mysqldump`, `pg_basebackup`. «Запустить `psql`», «через `pg_dump`».
...
7. **В одном тексте — один вариант для каждого термина.** Не смешивай «партиция» и «секция», «колонка» и «столбец». Shard — всегда «шард».
...
| HTAP (Hybrid Transactional/Analytical Processing) | HTAP | Гибридная СУБД с OLAP+OLTP. Мировые примеры: SAP HANA, SingleStore, TiDB, Oracle Exadata. Российские игроки развиваются недавно (YDB и др.; вендорские продукты — в дополнении компании) |
...
| Column store / columnar database | колоночная СУБД / колоночное хранилище | ClickHouse, Vertica, Greenplum |
...
| BASE | BASE | Basically Available, Soft state, Eventual consistency. Модели согласованности — см. Раздел 13 |
...
| Snapshot | снимок / snapshot / снапшот | «Снапшот» — устоявшийся вариант; для Tarantool канон — «снапшот» (вендорское дополнение `tarantool-translation-terms` выигрывает) |
...
| Commit a transaction | зафиксировать транзакцию / закоммитить транзакцию | «Транзакция зафиксирована»; в разговорном техтексте — «закоммитить» |
...
| Failover | аварийное переключение / failover | Для БД — переключение на реплику при отказе primary |
...
| Binlog (binary log) | binlog / бинарный лог | MySQL. «Включить binlog» |
...
| Orchestrator | оркестратор | Общее понятие: компонент, управляющий кластером и переключением. Утилита Orchestrator для MySQL — имя продукта, см. Раздел 10 |
| ProxySQL | ProxySQL | — |
...
| Shard | шард | Устоявшийся термин, в том числе в документации ClickHouse RU. Не «сегмент» |
...
| Distributed transaction | распределённая транзакция | — |
| Coordinator / Router | координатор / роутер | В шардированных кластерах — компонент, направляющий запрос к нужному шарду |
| Routing | маршрутизация (запросов) | — |
| Replica set | replica set / набор реплик | MongoDB; для Tarantool — «набор реплик» по вендорскому дополнению |
| Quorum | кворум | — |
...
| Table inheritance | наследование таблиц | Старый способ партиционирования в PostgreSQL |
...
| Orchestrator tool (MySQL) | Orchestrator | Имя продукта — HA-утилита для MySQL (топология и переключение primary), не переводится |
...
| Part | парт / кусок данных | Единица хранения MergeTree на диске внутри партиции; не «партиция» и не «сегмент» |
...
## Раздел 12. Вендорские дополнения

Терминология продуктов конкретного вендора (например, Tarantool: space/tuple, memtx/vinyl, VShard, Cartridge) вынесена из словаря движка в слой компании — скилл `tarantool-translation-terms` («translation» (внутренний материал системы, в эту выгрузку не входит), подключён переводчику как справочник). Форк под другую компанию заводит свой файл-дополнение той же формы.
...
| Eventual consistency | согласованность в конечном счёте / eventual consistency | Не «итоговая согласованность» |
...
| Aggregation pipeline (MongoDB) | конвейер агрегации / aggregation pipeline | — |
...
| English | Русский | Комментарий |
|---------|---------|-------------|
| to commit | закоммитить / зафиксировать | Транзакция в БД — «зафиксировать», см. «Commit a transaction» |
...
### psql / mysql / clickhouse-client / pg_dump
...
### 9. «Шард» и «парт» в ClickHouse

```
✗ «данные распределены по сегментам кластера», «партиции слились в фоне»
✓ «данные распределены по шардам кластера», «парты слились в фоне»
```

`Shard` — всегда «шард», в том числе в ClickHouse: «сегмент» — ошибочная калька. `Part` в ClickHouse — «парт» (кусок данных на диске внутри партиции), не «партиция» и не «сегмент»: партиция (`PARTITION BY`) — логическая группа партов.
...
- [ ] **Имена встроенных типов и движков** (`InnoDB`, `MergeTree`, `MyISAM`, `TOAST`, `BTREE`, `GIN`, `GiST`, `JSONB`, `UUID`) — оригинал
- [ ] **Команды CLI** (`psql`, `mysql`, `clickhouse-client`, `pg_dump`, `mysqldump`, `pg_basebackup`) — оригинал, строчная буква, не склоняются
- [ ] **Master/slave** — заменены на `primary/replica` (если оригинал современный); сохранены, если автор использует их сознательно
- [ ] **«Партиция/секция»** — в тексте один вариант; **shard** — всегда «шард», не «сегмент»; **part** в ClickHouse — «парт», не «партиция»
...
2. **Проверь публикации на Хабре от первоисточников:** habr.com/ru/companies/postgrespro/, habr.com/ru/companies/clickhouse/, habr.com/ru/companies/vk/
3. **Если нигде нет** — оставь в оригинале и при первом упоминании дай сноску переводчика (`[^N]` … «— Прим. перев.») — сноска, не скобки: `Write skew[^1]` → `[^1] Аномалия, при которой две транзакции читают пересекающиеся данные и пишут в разные строки; устоявшегося русского термина нет. — Прим. перев.`
