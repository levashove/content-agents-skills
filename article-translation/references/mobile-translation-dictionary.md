...
description: |
  ...
  технических материалах компании. Покрывает Swift/SwiftUI/UIKit, Kotlin/Jetpack
...
Словарь терминологии мобильной разработки (iOS, Android, кросс-платформенные фреймворки). Используется для перевода статей и для унификации терминологии в любых русскоязычных технических материалах компании.
...
4. **Названия системных компонентов — оригинал.** Activity, Fragment, ViewController, Intent, Service, BroadcastReceiver, ContentProvider. При первом упоминании узкого термина — сноска переводчика («… — Прим. перев.»), не скобки: `Intent[^1]` + текст сноски «Intent — объект-сообщение для запуска компонентов Android. — Прим. перев.».
...
7. **Российская специфика — точные русские названия.** RuStore, RuMarket; продуктовые названия MDM и картографических SDK — по вендорским скиллам `company/`, в движке не перечисляются. См. также `security-translation-dictionary`.
...
| Mobile app / application | мобильное приложение | — |
...
| Apple App Store | App Store | Не склоняется; общее «app store» → «магазин приложений», см. раздел 11 |
| App Store Connect | App Store Connect | — |
| Apple Developer | Apple Developer | — |
| Provisioning profile | provisioning profile | Оригинал; при первом упоминании — сноска переводчика. Не путать с общим provisioning («выделение ресурсов») |
...
| App Clip | App Clip | — |
| Widget | виджет | iOS — мини-приложение на экране (WidgetKit); Flutter — единица UI (StatelessWidget / StatefulWidget); Android — App Widget. См. раздел 18, п. 7 |
| Extension (iOS) | расширение / extension | App extension — «расширение приложения» (Share, Notification, Widget) |
...
| Constraint | ограничение / constraint | Auto Layout: ограничение раскладки; в коде — `NSLayoutConstraint` |
...
| Extension (Swift) | расширение / extension | Языковая конструкция Swift; в коде — `extension` |
...
| Bridge | мост | React Native bridge — «мост» между JS и нативным кодом |
...
| English | Русский | Комментарий |
|---------|---------|-------------|
| App store (generic) | магазин приложений | Общее понятие; конкретные магазины — по именам |
| Apple App Store | App Store | Не склоняется |
| Google Play / Play Store | Google Play / Play Store | — |
| **RuStore** | **RuStore** | Российский магазин приложений |
...
| App review | ревью приложения / модерация приложения | Apple: App Review — название процесса проверки в App Store |
...
| **MyTracker** | MyTracker | Российская аналитика |
...
| English | Русский | Комментарий |
|---------|---------|-------------|
| Russian app store | **RuStore**, **NashStore**, **RuMarket** | Российские магазины приложений |
| Russian mobile analytics | **AppMetrica, MyTracker** | — |
| Russian push delivery service | **RuStore Push, AppMetrica Push** | Альтернативы FCM/APNs |
| Russian biometric standards | **ЕБС (Единая биометрическая система)** | — |
| Domestic mobile OS | **ОС Аврора, РОСА Мобайл** | Российские мобильные ОС — официальные написания |
...
| to deploy (mobile) | развернуть / задеплоить / залить в магазин | «Задеплоить», «залить» — разг.; для магазина чаще «опубликовать» |
| to publish (app) | опубликовать (приложение) | — |
| to ship (release) | выпустить / зарелизить / выкатить | «Зарелизить», «выкатить» — разг. |
| to swipe | свайпнуть | — |
| to tap | тапнуть | — |
| to scroll | скроллить | — |
...
| **MyTracker Docs** | Российская мобильная аналитика |
...
Приоритеты при конфликте — в `references/translation-standards.md`.
