...
description: |
  ...
  на русский. Содержит: что переводится, что остаётся в оригинале, падежные
  формы и склонения, типовые ошибки. Подключается к article-translator,
...
2. **Русское слово по умолчанию, транслитерация — для частых терминов.** `pod` → «под», `node` → «узел» (в техтексте допустима «нода»). Используй в одном тексте **один** вариант, не смешивай «под» и `Pod`, «узел» и «нода».
...
5. **Аббревиатуры — оригинал.** API, RBAC, CRD, CRI, CNI, CSI, HPA, VPA, PV, PVC. При первом упоминании допустимо раскрыть сокращение рядом с ним — «RBAC (Role-Based Access Control)»: это расшифровка аббревиатуры, а не толкование термина.
6. **При первом упоминании узкого термина — сноска переводчика, не скобки.** В тексте — `Sidecar[^1]`, `Taint[^2]`; в конце материала — `[^1]: Sidecar — вспомогательный контейнер в том же поде. — Прим. перев.`, `[^2]: Taint — метка на узле, запрещающая размещать на нём поды без соответствующего toleration. — Прим. перев.` Дальше — термин без сноски. Три исхода для любого термина: русское слово, оригинал, оригинал со сноской.

## Раздел 1. Базовые объекты Kubernetes API

| English | Русский | Комментарий |
|---------|---------|-------------|
| Pod | под / `Pod` | В нарративе — «под» (склоняется: пода, поду; мн.ч. — поды). Имя ресурса API — `Pod` с заглавной, как в YAML, не склоняется. Один вариант на текст |
| Node | узел / нода | «Узел» — официальный перевод CNCF; в техтексте допустима «нода». Один вариант на текст |
| Cluster | кластер | Склоняется: кластера, кластеру, кластером |
| Namespace | пространство имён / namespace | В командах и YAML — `namespace`; в нарративе — «пространство имён» |
| Container | контейнер | Базовый перевод, без вариантов |
| Image | образ | «Образ контейнера», не «имидж» |
| Container Image | образ контейнера | — |
| Registry | реестр | «Реестр образов» (container registry). `registry` — только в именах продуктов: Docker Registry |
...
| Label | метка | В нарративе — «метка»; в полях YAML — `labels`, `matchLabels`. Не дублировать оригиналом рядом с русским словом |
...
| English | Русский | Комментарий |
|---------|---------|-------------|
| Deployment | Deployment | Объект API — не переводится, не склоняется. «Деплоймент» — разговорно, в статьях — `Deployment` |
| Deployment process (action) | развёртывание | Действие, а не объект API: «развёртывание приложения», «поэтапное развёртывание». «Деплой» — разговорный англицизм техтекста |
...
| Rollout | выкатка / rollout | «Выкатка обновления» — в практическом тексте; `rollout` — в командах: `kubectl rollout status` |
| Rollback | откат | «Откат к предыдущей версии»; глагол — «откатить». `rollback` — только в командах |
| Rolling update | поэтапное обновление / rolling update | В нарративе — «поэтапное обновление»; `RollingUpdate` — значение поля `strategy.type` |
| Canary deployment | канареечное развёртывание | Не «canary-деплой», не «канареечная выкатка» |
| Blue/Green deployment | blue/green-развёртывание | Цветовая пара остаётся латиницей, через дефис; не «сине-зелёное развёртывание» |
...
| Headless Service | headless Service | Оригиналом; описательные варианты вроде «бесшапочный» не прижились |
| Ingress | Ingress | — |
...
| `Ambient Mode` (Istio) | `Ambient Mode` / амбиент-режим | Архитектура Istio без sidecar. При первом упоминании — оригинал, далее допустим «амбиент-режим» |
| `Sidecar Mode` (Istio) | `Sidecar Mode` / sidecar-режим | Традиционная архитектура Istio. При первом упоминании — оригинал, далее допустим «sidecar-режим» |
| `Waypoint proxy` | `Waypoint proxy` / waypoint-прокси | L7-прокси на service account в Ambient. При первом упоминании — оригинал, далее допустим «waypoint-прокси» |
| `ztunnel` | `ztunnel` | L4-прокси на узле в Ambient (zero-trust tunnel). Не склонять |
...
| Policy attachment (паттерн Gateway API) | policy attachment | Латиницей; при первом упоминании — сноска переводчика: «Policy attachment — паттерн Gateway API для привязки политик к ресурсам. — Прим. перев.» |
...
| Sidecar | sidecar / сайдкар | При первом упоминании — сноска переводчика: «Sidecar — вспомогательный контейнер в том же поде. — Прим. перев.» |
| CNI (Container Network Interface) | CNI | Аббревиатура; при первом упоминании — расшифровка |
...
| CSI (Container Storage Interface) | CSI | Аббревиатура; при первом упоминании — расшифровка |
...
| ConfigMap | ConfigMap | Имя ресурса API — не переводить |
| Secret | Secret | Имя ресурса API — не «секрет» в кавычках, а `Secret` |
| Resource quota | ResourceQuota / квота ресурсов | — |
| LimitRange | LimitRange | — |
| Requests / Limits | requests / limits | Не переводить — это поля YAML |
| QoS (Quality of Service) Class | QoS Class / класс QoS | Аббревиатура; при первом упоминании — расшифровка |
...
| Admission Controller | admission controller | Оригиналом, строчными; при первом упоминании — сноска переводчика: «Admission controller — компонент Kubernetes, проверяющий и изменяющий запросы к API до записи объекта. — Прим. перев.» |
...
| Master node | мастер-нода / master-нода | Через дефис. Термин устаревает в пользу control plane node → «узел управляющего слоя» |
| Worker node | рабочий узел / рабочая нода / worker-нода | Согласуй с вариантом, выбранным для node в этом тексте |
...
| English | Русский | Комментарий |
|---------|---------|-------------|
| CRD (Custom Resource Definition) | CRD | Аббревиатура; при первом упоминании — расшифровка |
...
## Раздел 10. Планирование подов и ресурсы узлов

| English | Русский | Комментарий |
|---------|---------|-------------|
| Scheduling | планирование | «Pod scheduling» → «планирование подов» |
| Affinity / Anti-Affinity | affinity / anti-affinity | Оригиналом; при первом упоминании — сноска переводчика: «Affinity — правила привязки подов к узлам или друг к другу. — Прим. перев.» |
| Node Affinity | node affinity | — |
| Pod Affinity | pod affinity | — |
| Taint | taint | Оригиналом; при первом упоминании — сноска переводчика: «Taint — метка на узле, запрещающая размещать на нём поды без соответствующего toleration. — Прим. перев.». Описательный вариант «метка отторжения» не устоялся |
| Toleration | toleration | Оригиналом, парный к taint; «толерантность» не используется |
...
| Drain | drain / осушение узла | «Сделать `kubectl drain`», «осушить узел» |
| Cordon | cordon | «Сделать `kubectl cordon`», «запретить размещение новых подов на узле» |
| Preemption | preemption / вытеснение по приоритету | — |
| PriorityClass | PriorityClass | — |
| Probe (Liveness / Readiness / Startup) | Liveness/Readiness/Startup Probe / проверка работоспособности | При первом упоминании — сноска переводчика: «Probe — проверка состояния контейнера: liveness — жив ли, readiness — готов ли принимать трафик, startup — завершился ли запуск. — Прим. перев.» |
...
| English | Русский | Комментарий |
|---------|---------|-------------|
| Observability | наблюдаемость / observability | Оба варианта приемлемы, один на текст. `observability` — допустимый англицизм в техтексте |
| Metrics | метрики | — |
| Logs | логи | — |
...
| Dashboard | дашборд / панель | «Дашборд» — устоявшаяся транслитерация |
...
| Kubernetes-native | Kubernetes-native / нативный для Kubernetes | Термин знаком аудитории — не дублировать русским вариантом рядом: «Kubernetes-native операторы (нативные для Kubernetes)» — избыточно |
| Container Orchestration | оркестрация контейнеров | — |
| Microservices | микросервисы | — |
| Serverless | serverless / бессерверный | Чаще «serverless»: «serverless-функции»; в нарративе допустимо «бессерверные вычисления» |
| Function-as-a-Service (FaaS) | FaaS | — |
| Multi-tenancy | мультитенантность / multi-tenancy | — |
| Multi-cluster | мультикластерный / multi-cluster | «Мультикластерная установка»; слитно, без дефиса |
...
### ИИ-агенты на Kubernetes (2026)

Продукты и сущности агентного слоя. Все — латиницей, не транслитерировать
(«актор», «воркер»), не склонять, множественное число английским `-s`:
`Actors`, `Workers`, `WorkerPools`, `ActorTemplates`. Исходная статья —
InfoQ «Pods as Workers, Not Agents».

| English | Русский | Комментарий |
|---------|---------|-------------|
| kagent | kagent | Не переводить. CNCF-проект (Solo.io): запуск и оркестрация ИИ-агентов как нативных ресурсов Kubernetes. **Всегда строчными**, включая начало предложения — если по пунктуации нельзя, перестроить фразу. Узкий термин: при первом упоминании — сноска |
| Agent Substrate | Agent Substrate | Не переводить. Control plane Google поверх Kubernetes: планирует логические агенты (Actor) на пул исполнителей (Worker). Узкий термин: при первом упоминании — сноска |
| Agent Sandbox | Agent Sandbox | Не переводить. Проект экосистемы Kubernetes (`sigs.k8s.io`): изолированная песочница для выполнения кода ИИ-агентов. Узкий термин: при первом упоминании — сноска. Полное имя «Kubernetes Agent Sandbox project» — при первом упоминании, далее сокращать |
| Actor (Agent Substrate) | Actor | Не переводить и не транслитерировать. Логический ИИ-агент, планируется на Worker. Не путать с моделью акторов и с `Actor` из Swift Concurrency (mobile-словарь, там → «актор») |
| Worker (Agent Substrate) | Worker | Не переводить. Единица-исполнитель, 1:1 с Pod. **Не путать с Worker node** — это «рабочий узел / worker-нода» (раздел 7). В одном абзаце держать разграничение явно |
| WorkerPool | `WorkerPool` | Ресурс API Agent Substrate, группа Worker; аналог NodePool. В оригинале — в backtick'ах, сохранять моноширинный формат, не склонять |
| ActorTemplate | `ActorTemplate` | Ресурс API Agent Substrate, декларативное описание Actor; аналог спецификации Pod. В оригинале — в backtick'ах, сохранять формат, не склонять |
| NodePool | NodePool | Оригиналом, сноска не нужна: устоявшийся термин managed-Kubernetes у облачных провайдеров; в русских текстах также «пул узлов» |
...
| to provision | выделить / подготовить | «Выделить узлы под кластер»; provisioning → «выделение ресурсов» |
...
| to taint a node | поставить taint на узел | — |
...
Канонический вариант — «узел»; «нода» допустима в техтексте. В одном тексте — один вариант.
...
- [ ] **Узкие термины (sidecar, taint, admission controller)** — оригинал со сноской переводчика «— Прим. перев.» при первом упоминании, без пояснений в тексте
...
Если в русскоязычном сообществе K8s нет единого варианта (например, `node` → «узел» / «нода», `observability` → «наблюдаемость» / `observability`), действуй по приоритету:
...
3. **Если переводишь в первый раз — бери канонический (первый) вариант словаря:** «узел», «под», «наблюдаемость». «Нода» и `observability` — осознанный выбор под площадку, не дефолт.
...
Версия словаря: **2026-09-17**. Обновляется по мере появления новых терминов и устоявшихся переводов.
