...
description: |
  ...
  с английского на русский. Покрывает CIA triad, типы атак (phishing,
...
5. **Российская регуляторика — точные русские названия.** 152-ФЗ, 187-ФЗ, ФСТЭК, ФСБ, КИИ, ГосСОПКА, БДУ ФСТЭК. Не переводить английскими аналогами.
...
8. **Три исхода для узкого термина.** Термин либо переводится, либо остаётся в оригинале, либо остаётся в оригинале со сноской переводчика при первом упоминании: `admission controller[^1]` → `[^1]: Admission controller — компонент Kubernetes, проверяющий запросы к API до сохранения объекта. — Прим. перев.` Пояснений и оригинала в скобках инлайн — нет.
...
| Hardening | усиление защиты / харденинг | — |
...
| Payload | полезная нагрузка / payload | В контексте эксплойта — «нагрузка эксплойта» |
...
| Defense in depth | эшелонированная защита | Не «оборона в глубину» |
...
| Key derivation | формирование ключа / вывод ключа / key derivation | Не «производство ключа» |
| KDF (Key Derivation Function) | KDF / функция формирования ключа | PBKDF2, scrypt, Argon2 |
...
| Security code review | ревью кода на безопасность / security code review | Не путать с обычным code review |
...
| Cloud privilege escalation | повышение привилегий в облаке | Через IAM-политики, роли, метаданные инстансов |
...
| Admission controller | admission controller | Оригинал; при первом упоминании — сноска переводчика. Не «контроллер допуска» |
...
| ServiceAccount (K8s object) | ServiceAccount | Имя объекта API — оригинал, слитно. Общее понятие — см. Раздел 7 |
...
| Dependency confusion | путаница зависимостей / dependency confusion | Устоявшийся ru-вариант на Хабре. При первом упоминании — сноска переводчика с оригиналом |
| Typosquatting | тайпсквоттинг | Правильно «тайпсквоттинг» — не «тайпосквоттинг» (ru.wikipedia, encyclopedia.kaspersky.ru) |
...
| SHA pinning / pin by digest | закрепление по SHA / SHA-пиннинг | При первом упоминании — сноска переводчика: «закрепление action по полному SHA коммита». «Pin by digest» в OCI — «пин по digest» |
...
| Scoped secrets | секреты с ограниченной областью действия / scoped-секреты | Первое упоминание — полный вариант; далее «scoped-секреты» |
...
| Federal Service for Technical and Export Control | **ФСТЭК** | Официально «ФСТЭК России»; после первого упоминания достаточно «ФСТЭК» |
...
| Personal data protection level (УЗ) | **уровень защищённости персональных данных УЗ-1…УЗ-4** | Приказ ФСТЭК № 21; ранее — классы ИСПДн К1–К4 по приказу № 58 (отменён). Не «класс защиты СЗИ» |
| Protection class (СЗИ) | **класс защиты СЗИ (1–6)** | Требования ФСТЭК к СЗИ: 6 классов, 1-й — наивысший; в ГИС и ИСПДн применяются 4–6 |
...
| Adversarial attack | состязательная атака / adversarial attack | Не «атака с противоборством» |
| Adversarial example | состязательный пример / adversarial example | — |
...
| to harden | усилить защиту / захарденить (разг.) | — |
| to scan | сканировать / просканировать | — |
...
- **ФСТЭК** (ж.р., Федеральная служба): не склоняется в официальных и технических текстах — «приказ ФСТЭК России», «требования ФСТЭК», «сертификат ФСТЭК России»; разговорное «ФСТЭКом» — не использовать
...
6. Если нигде нет — оставь в оригинале; при первом упоминании — сноска переводчика («… — Прим. перев.»), не скобки
...
| **Российские ГОСТ Р по ИБ** | ГОСТ Р 34.10, ГОСТ Р 34.11, ГОСТ Р ИСО/МЭК 27001 (адаптация ISO 27001) |
