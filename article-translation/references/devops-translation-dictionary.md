...
| Render / rendered configuration | рендерить / отрендеренная конфигурация | НЕ синоним `generate`: рендеринг обратим (шаблон → манифест и назад), генерация однонаправленна. Подмена одного другим стирает различие, на котором держатся тексты про IaC-абстракции |
| Unidirectional generator | однонаправленный генератор | Противопоставляется двунаправленному «представлению» (view) — см. `View (over configuration)` |
| View (over configuration) | представление | Метафора из СУБД (DB VIEW), растянутая на архитектуру абстракций: двунаправленное окно в эталонные данные. Парный термин — `projection` → «проекция». Держать сквозняком: разнобой «представление / вид / взгляд» рушит оппозицию с генератором |
