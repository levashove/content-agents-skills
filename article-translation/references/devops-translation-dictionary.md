...
description: |
  ...
  на русский. Покрывает CI/CD, Infrastructure as Code (Terraform,
...
3. **Базовые понятия — переводятся.** «Конвейер» (pipeline), «развёртывание» (deployment как действие), «откат» (rollback), «сборка» (build), «пулреквест» (pull request), «реестр» (registry), «наблюдаемость» (observability). Русское слово — по умолчанию; оригинал — только для имён продуктов, команд и полей кода.
...
5. **Аббревиатуры — оригинал.** CI/CD, IaC, SRE, SLO/SLA/SLI, MTTR, MTBF, RPO/RTO, DORA. При первом упоминании — полное название и аббревиатура (см. ряд `Continuous Integration (CI)`).
6. **В одном тексте — один вариант для каждого термина.** Не смешивай `pipeline` и «конвейер» в соседних абзацах.
7. **Пояснения — только сносками.** Термин либо переводится, либо остаётся в оригинале, либо остаётся в оригинале со сноской переводчика (`[^N]` … «— Прим. перев.»). При первом упоминании узкого термина — сноска переводчика, не скобки: `Golden path[^1]`, где `[^1]: Golden path — эталонный, поддерживаемый платформой путь разработки. — Прим. перев.`
...
| DevOps engineer | DevOps-инженер | Через дефис, инженер склоняется. «Девопс» — разговорно |
...
| Platform Engineering | платформенная инженерия / platform engineering | Русский вариант устоялся (Хабр, ру-доки) — давать сразу по-русски; оригинал — при повторах в техтексте |
| Golden path | эталонный путь / golden path | Узкому читателю при первом упоминании — сноска переводчика («… — Прим. перев.»), не скобки |
...
| Self-service | самообслуживание / self-service | «Выделение ресурсов в режиме самообслуживания», «интерфейс самообслуживания» |
...
| Shift Left | shift left / «сдвиг влево» | При первом упоминании — сноска переводчика |
...
| English | Русский | Комментарий |
|---------|---------|-------------|
| Pipeline | конвейер / CI/CD-пайплайн | «Конвейер» — канон; в составных допустим «CI/CD-пайплайн», «pipeline» — в коде и именах (`.gitlab-ci.yml` pipeline). В одном тексте — один вариант |
| Stage | этап / stage | — |
...
| Pull Request (PR) | пулреквест / PR | При первом упоминании — «пулреквест (PR)», при повторах — PR. На GitLab это Merge Request — см. тонкость ниже |
| Merge Request (MR) | мержреквест / MR | В GitLab — MR. Не «MR в GitHub» — там это PR |
| Code Review | код-ревью / ревью кода | «Сделать ревью», «пройти ревью». Ревью безопасности кода — см. security-словарь |
...
| English | Русский | Комментарий |
|---------|---------|-------------|
| Rolling update | поэтапное обновление / rolling update | В нарративе — «поэтапное обновление»; `RollingUpdate` как значение поля Kubernetes — оригинал |
| Blue/Green deployment | blue/green-развёртывание / blue/green-деплой | Не переводить «синий/зелёный» |
| Canary deployment | канареечное развёртывание / канареечный деплой | «Канареечный деплой» — разговорно, в техтексте допустимо |
...
| Shadow deployment | теневое развёртывание / shadow deployment | — |
...
| Provisioning | выделение ресурсов / provisioning | Русский вариант — сразу; «provisioning» допустим как англицизм в техтексте, «провижининг» не использовать. Ср. `to provision` |
| Drift | drift / расхождение конфигурации | При первом упоминании — сноска переводчика |
...
| Source of truth | источник истины | Устоявшийся термин (SSOT — single source of truth). «Единый источник истины» — когда в оригинале `single` |
| Authoritative record / copy | эталонная копия | Не «авторитетная»: по-русски читается как «имеющая вес», а не «достоверная». `record` и `copy` в англоязычных текстах взаимозаменяемы — переводить одинаково, не различая «запись» и «копию» |
| Render / rendered configuration | рендерить / отрендеренная конфигурация | Не синоним `generate`: рендеринг обратим (шаблон ↔ манифест), генерация однонаправленна |
| Unidirectional generator | однонаправленный генератор | Антоним — двунаправленное «представление», см. `View (over configuration)` |
| View (over configuration) | представление | Метафора из СУБД: двунаправленное окно в эталонные данные; парный термин `projection` → «проекция»; в одном тексте — один вариант (не «вид», не «взгляд») |
| Variant / variant generation | вариант / генерация вариантов | Термин Брайана Гранта (IaC): порождение окружений-вариантов из общей базы |
| Repeatability | повторяемость | В IaC-текстах обычно раскрывается через возможность создавать варианты — связку сохранять явной |
| Internal developer platform (IDP) | внутренняя платформа разработки | Аббревиатуру IDP вводить только при повторных упоминаниях. Не путать с IdP (Identity Provider) — см. security-словарь |
...
| Module | модуль | «Terraform-модуль», «модуль Ansible»; в коде — `module` |
...
| Lock | блокировка / lock | «State lock» → «блокировка state»; файл `.terraform.lock.hcl` — «lock-файл» |
...
| Plugin | плагин | — |
| Terraform Registry | Terraform Registry | Имя сервиса — не переводить; в нарративе — «реестр модулей». Общее понятие — ряд `Registry` в разделе 11 |
| OpenTofu | OpenTofu | Форк Terraform — не переводить |
| Lifecycle | жизненный цикл / lifecycle | В коде — блок `lifecycle` |
...
| Playbook | плейбук / playbook | «Ansible playbook» → «плейбук Ansible». В SRE-контексте (сценарий реагирования на инцидент) — тоже «плейбук»; пошаговая инструкция — `Runbook` |
...
| Digest | digest / хеш | — |
| Registry | реестр / registry | «Container registry» → «реестр образов»; `registry` — только в именах (Docker Registry, Terraform Registry) |
...
| Observability | наблюдаемость / observability | «Observability» — при повторах в техтексте |
...
| Test coverage | покрытие тестами | «Code coverage» → «покрытие кода» |
...
| Quay | Quay | Имя продукта — реестр образов Red Hat |
| Trivy | Trivy | Имя сканера уязвимостей (Aqua Security) |
| DCO sign-off | DCO sign-off | Не переводить аббревиатуру: Developer Certificate of Origin, строка `Signed-off-by` |
...
| English | Русский | Комментарий |
|---------|---------|-------------|
| Annotation sprawl | зоопарк аннотаций / разрастание аннотаций | «Зоопарк» — где автор передаёт раздражение; «разрастание» — нейтрально. При первом упоминании — сноска переводчика с оригиналом («annotation sprawl — Прим. перев.»), не скобки |
| Impedance mismatch | несоответствие моделей | При первом упоминании — сноска переводчика с оригиналом («impedance mismatch — Прим. перев.»), не скобки. «Рассогласование» — не использовать |
...
| Blast radius | радиус поражения | При первом упоминании — сноска переводчика с оригиналом («blast radius — Прим. перев.»), не скобки |
...
| to provision | выделить (ресурсы) / подготовить (инфраструктуру) | Ср. `Provisioning` → «выделение ресурсов»; «провижионить» не использовать |
| to commit | закоммитить / зафиксировать | «Закоммитить изменения» — техтекст; «зафиксировать» — нейтральный стиль. Фиксация транзакции в БД — см. databases-словарь |
...
| to build | собрать | — |
| to ship | выпустить / поставить | «Выпустить релиз», «поставить фичу» |
...
### Pipeline → конвейер / CI/CD-пайплайн

| Форма | Конвейер | Пайплайн (в составных) |
|-------|----------|------------------------|
| Им. | конвейер | CI/CD-пайплайн |
| Род. | конвейера | CI/CD-пайплайна |
| Дат. | конвейеру | CI/CD-пайплайну |
| Мн.ч. | конвейеры | CI/CD-пайплайны |

`pipeline` латиницей — только в коде и именах (`pipeline:` в `.gitlab-ci.yml`), не склоняется.
...
- [ ] **Согласованность транслитерации** — в тексте один вариант (либо «конвейер», либо «CI/CD-пайплайн»)
...
4. **Если нигде нет** — оставь **в оригинале**; если читателю нужно пояснение — сноска переводчика (`[^N]` … «— Прим. перев.»), не скобки и не инлайн-заметка в тексте.
...
Версия словаря: **2026-09-17**. Обновляется по мере появления новых терминов.
...
- **Смежные словари** (`databases-`, `security-`, `networking-translation-dictionary`) — подключаются параллельно при пересечении тем; при коллизии терминов первый вариант — по Общей таблице `references/translation-glossary.md`.
