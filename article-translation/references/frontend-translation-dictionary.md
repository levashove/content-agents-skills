...
description: |
  ...
  с английского на русский и унификации терминов в русскоязычных
  технических материалах компании. Покрывает HTML/CSS/JS, React/Vue/Angular/Svelte/Solid,
...
Словарь терминологии фронтенд-разработки. Используется для перевода статей и для унификации терминологии в любых русскоязычных технических материалах компании.
...
2. **Имена API — оригинал.** Fetch API, Web Components, IndexedDB, Service Worker, Shadow DOM. При первом упоминании узкого термина — сноска переводчика («… — Прим. перев.»), не скобки: `Service Worker[^1]` + текст сноски.
...
6. **Аббревиатуры — оригинал.** DOM, BOM, JSX, TSX, SPA, MPA, PWA, SSR, SSG, ISR, CSR, ESM, CJS, AMD, UMD, BEM, OOCSS. Расшифровка при первом упоминании — сноской переводчика, не в скобках.
...
| Backend | бэкенд | — |
| Full-stack | full-stack / фулстек | Оригинал; «фулстек» (одна «л») — разг. |
...
| Cross-browser | кроссбраузерный | Пишется слитно |
...
| Lazy loading | ленивая загрузка / lazy loading | Для изображений — `loading="lazy"` |
...
| TypeScript strict mode | строгий режим TypeScript | `"strict": true` в tsconfig; не путать с React Strict Mode |
...
| Rendering | рендеринг | — |
| Re-render | повторный рендер / ререндер | Без дефиса после приставки |
...
| Strict Mode | Strict Mode | Имя API React (`<StrictMode>`), оригинал; «строгий режим» — пояснение, не термин |
| Suspense | Suspense | — |
...
| Router | роутер / маршрутизатор | Роутер SPA; React Router, Vue Router — имена, оригинал. Сетевой маршрутизатор — `networking-translation-dictionary` |
...
| English | Русский | Комментарий |
|---------|---------|-------------|
| Unit test | юнит-тест | Не «unit-тест» |
...
| Image lazy loading | ленивая загрузка изображений | `loading="lazy"` |
...
| to re-render | отрендерить заново / ререндерить | Без дефиса после приставки |
...
| to deploy | развернуть / задеплоить | «Задеплоить» — разг. |
