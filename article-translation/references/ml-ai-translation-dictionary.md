...
description: |
  ...
  с английского на русский. Покрывает классическое ML, deep learning,
...
| Streaming | стриминг / потоковый вывод | В контексте LLM-ответа — «потоковая генерация» |
...
| Thinking model / Reasoning model | рассуждающая модель / reasoning-модель | OpenAI o1/o3, Claude с расширенным рассуждением (extended thinking) |
| Extended thinking | расширенное рассуждение / extended thinking | Режим рассуждающих моделей; не «расширенное мышление» |
...
| LLMOps | LLMOps | — |
| Notebook (Jupyter и т. п.) | ноутбук / notebook | В ML-контексте «ноутбук» устоялся (Jupyter-ноутбук); при риске путаницы с laptop — «Jupyter-ноутбук» или «notebook-сервер». Склонение «notebook-и / notebook-ов» не использовать — либо русский «ноутбук», либо составное «notebook-сервер» |
...
| Streaming inference | потоковый инференс / streaming inference | — |
| Model A/B testing | A/B-тестирование моделей | — |
| Shadow deployment | теневое развёртывание / shadow deployment | Модель получает реальный трафик, но её ответы не отдаются пользователю |
...
| Perplexity AI | Perplexity | Поисковый ИИ-сервис; не путать с метрикой perplexity (раздел 9) |
...
| Adversarial attack | состязательная атака / adversarial attack | — |
...
| to prompt | задать промпт / отправить промпт | «Запромптить» — разг., в текстах не использовать |
...
| to deploy (a model) | развернуть (модель) / задеплоить (разг.) | «Развернуть модель» — в текстах; «задеплоить» — разговорно |
...
4. Если нигде нет — оставь в оригинале; при первом упоминании узкого термина — сноска переводчика (`[^N]` … «— Прим. перев.»), не скобки
